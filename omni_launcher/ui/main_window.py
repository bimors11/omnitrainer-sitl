...
class MainWindow(QMainWindow):
    ...
    def _build_side_panel(self) -> QWidget:
        ...
        buttons_1 = QGridLayout()
        start_sitl_btn = QPushButton("Start SITL")
        start_efi_btn = QPushButton("Start EFI")
        start_rangefinder_btn = QPushButton("Start RNG")
        start_all_btn = QPushButton("Start All")
        stop_all_btn = QPushButton("Stop All")
        start_sitl_btn.clicked.connect(self.start_sitl_clicked)
        start_efi_btn.clicked.connect(self.start_efi_clicked)
        start_rangefinder_btn.clicked.connect(self.start_rangefinder_clicked)
        start_all_btn.clicked.connect(self.start_all_clicked)
        stop_all_btn.clicked.connect(self.stop_all)
        buttons_1.addWidget(start_sitl_btn, 0, 0)
        buttons_1.addWidget(start_efi_btn, 0, 1)
        buttons_1.addWidget(start_rangefinder_btn, 0, 2)
        buttons_1.addWidget(start_all_btn, 1, 0, 1, 2)
        buttons_1.addWidget(stop_all_btn, 1, 2)
